...
hyperparams = {
    ...
    'branch_size_IC':[100,100,100,100], # hidden size of the branch modeled as IC
    'branch_size_FF':[100,100,100,100], # hidden size of the branch modeled as FF
...
def main():
    ...
    run = wandb.init(project='LIF_L1_DON', name=sys_args.name, config=hyperparams)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    # train_loss, train_loss_data = validate(train_dl,data['Locations'],train_loss_fn, 1, device)

    val_data = TrajectoryDataset(data["val"],max_seq_len=wandb.config['max_seq_len'],n_samples=wandb.config['n_samples'])
    test_data = TrajectoryDataset(data["test"],max_seq_len=wandb.config['max_seq_len'],n_samples=wandb.config['n_samples'])
    # return
    ...
    model_args = {
        ...
        'branch_size_FF': wandb.config['branch_size_FF'],
    ...
    model = DeepONet(**model_args,trunk_model=trunk_model)
    model.to(device)
    ...
    for epoch in range(wandb.config['n_epochs']):
        ...
        if early_stop.best_model:
            ...
            with torch.no_grad():
                y,x0_feed,t_feed,u_feed,deltas_feed,lengths,u = trajectory(data['test'],0,delta=1) # delta is hardcoded
                y_tot = []
                for batch_index, length in enumerate(lengths):
                    x0_batch = x0_feed[batch_index]
                    ...
                    for i in range(length): # Here DeepONet works as iterative solver, i.e: it takes its own output as input for the next step
                        u_input = u_batch[i]
                        y_pred,basis_functions = model(x0_batch.to(device), u_input.to(device), data['Locations'].to(device))
                        x0_batch = y_pred
                    y_tot.append(y_pred)

                y_pred = torch.stack(y_tot, dim=0)
