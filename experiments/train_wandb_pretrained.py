...
def main():
    ...
    for epoch in range(wandb.config['n_epochs']):
        ...
        if early_stop.best_model:
            ...
            y_pred, basis_functions = model(x0_feed.to(device), u_feed.to(device),data['Locations'].to(device),deltas_feed.to(device),kernel_pars.to(device),basis_functions.to(device))
