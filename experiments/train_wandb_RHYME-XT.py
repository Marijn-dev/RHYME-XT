...
hyperparams = {
    ...
    'trunk_modes_svd':20,                  # Number of modes used from SVD, max value = min(Nx,trunk_modes_svd)   
...
def main():
    ...
    run = wandb.init(project='RHYME-XT',entity='aguiar-kth-royal-institute-of-technology', name=sys_args.name, config=hyperparams)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    locations_offline = data['Locations_offline']*scaling
    locations_online = data['Locations_online']*scaling
    ...
    # x_in = locations
    x_out_train = x_out_test = x_in = locations_online
    ...
    if sys_args.pretrained_trunk == False:
        ...
        PHI = data["U"][:,:wandb.config['trunk_modes_svd']].to(device)
        print('Training on ground truth PHI with shape:', PHI.shape)
        locations = locations_offline.view(-1,1).to(device)
        for epoch in range(0,wandb.config['n_epochs_trunk']):
            ...
            # Wandb logging
            wandb.log({
            'Basis function FNN/epoch': epoch + 1,
            'Basis function FNN/Total_Loss': total.item(),
            'Basis function FNN/Data_Loss': rec_loss.item(),
            'Basis function FNN/Orthogonal_Loss': ortho_loss.item(),
            'Basis function FNN/Norm_Loss': norm_loss.item(),
        })
    ...
    for epoch in range(wandb.config['n_epochs']):
        ...
        wandb.log({
            ...
            'RHYME-XT/lr': optimiser.param_groups[0]["lr"],
