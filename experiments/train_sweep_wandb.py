...
def train_main():
    ...
    run = wandb.init(project='flumen_spatial_sweep_projecttrunk_CONV', name=sys_args.name, config=wandb.config)
...
if __name__ == '__main__':
    ...
    'parameters': {
        ...
        'control_rnn_depth': {'values': [1,2, 4, 6]},  # Control the depth of the RNN
        'encoder_size': {'values': [1, 2,4,6]},  # Encoder size (adjust accordingly)
        'encoder_depth': {'values': [2, 2, 4,6]},  # Encoder depth options
    ...
    sweep_id = wandb.sweep(sweep_configuration, project="flumen_spatial_sweep_projecttrunk_CONV")
